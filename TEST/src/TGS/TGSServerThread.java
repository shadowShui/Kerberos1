...
public class TGSServerThread extends Thread {
	...
	private static final long key_tgs = 456;
	private final long Ekcv = 999;	// v至C会话秘钥
    ...
    private TGSFrame tgsFrame;
    
    /** c->v 密码
     * @param tgsFrame */
    
    public TGSServerThread(Socket socket, TGSFrame tgsFrame) {
    
        this.socket = socket;
        this.init();
        this.tgsFrame = tgsFrame;
    }
	...
	private byte[] createMsgToClient(byte[] clientMsg) {
		...
		//输出log
		String log = Log.logC2T(".\\logC2T.txt",clientMsg,ticket_tgs,authenticator_c);
		this.tgsFrame.append(log);
...
//        Log.println("usr: " + usr);
        ...
        log = Log.logT2C(".\\logT2C.txt", ticket_v, res_t, res);
        this.tgsFrame.append(log);
